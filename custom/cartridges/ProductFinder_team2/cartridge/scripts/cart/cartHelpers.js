...
function hasSameOptions(existingOptions, selectedOptions) {
    ...
    for (var i = 0, j = selectedOptions.length; i < j; i++) {
        if (selectedOptions[i].optionId=='EngravingCustomisation') {
            selected[selectedOptions[i].optionId] = [selectedOptions[i].selectedValueId,selectedOptions[i].message];
        }else{
    ...
    return collections.every(existingOptions, function (option) {
           if (option.optionID=='EngravingCustomisation') {
            if (option.productID=='basicEngraving') {
                
                return option.parent.custom.engravingInput==selected[option.optionID][1];
            }
            else{
                return option.optionValueID === selected[option.optionID][0];
               }
           }
           else{
...
function addLineItem(
    ...
    try {
        productLineItem.custom.engravingInput  = customMsg;
    } catch (error) {
 ...
 *     has the same bundled items or options
 */
function getExistingProductLineItemInCart(product, productId, productLineItems, childProducts, options,customMsg) {
 ...
 */
function addProductToCart(currentBasket, productId, quantity, childProducts, options,customMsg) {
    var availableToSell;
    ...
    productInCart = getExistingProductLineItemInCart(
        product, productId, productLineItems, childProducts, options,customMsg);
